...
#input file path
path = '/bundle/data/Mu2e/MuEDM/NewFiles/'
...
for fNum in range(1,nfiles):
    ...
    filename = 'musr_'+str(inputFileNum)+'_'+ fNumStr +'.root' #EDM = 0.0
...
freqArr = np.array(freq[1:], dtype='float32')
freqErrorArr = np.array(freqError[1:], dtype='float32')

'''
freqArrnoB = np.delete(freqArr, 1)
freqErrorArrnoB = np.delete(freqErrorArr, 1)

countsFreqnoB, binEdgesFreqnoB = np.histogram(tot_posIniMom_array, bins = 5, range=(0, p_max))
binCentresFreqnoB = (binEdgesFreqnoB[:-1] + binEdgesFreqnoB[1:]) / 2
fig = plt.figure()
ax = fig.add_subplot(1, 1, 1)
plt.errorbar(binCentresFreqnoB, freqArrnoB, yerr=freqErrorArrnoB, fmt='.')                                                                                                                                 
plt.xlabel('Positron Momentum [Mev/c]')
plt.ylabel('Frequency of Polarisation Vector Oscillation [rad/$\mu$s]')
plt.savefig(outputDirTop + 'freq_momNoB.png')
'''
...
# Run fit
#output = least_squares(calcChiSq, pInit, args = (xdata, ydata, xerror, yerror))
outputFreq = least_squares(calcChiSq, pInit, args = (xdata, ydata, xerror, yerror), full_output=True)
xFreq = outputFreq[0]
cov_xFreq = outputFreq[1]                
...
# Get errors from our fits using fitStdError(), defined above
pErrorsFreq = cov_xFreq #fitStdError(output.jac)
#d_mFreq = np.sqrt(pErrorsFreq[0][0])
d_cFreq = np.sqrt(pErrorsFreq[0][0])

# Calculate fitted y-values using our fit parameters and the original fit function
xPlotFreq = np.linspace(np.min(xdata), np.max(xdata), 300)
fitDataFreq = fitFunc(xFreq, xPlotFreq)   

fig = plt.figure()
ax = fig.add_subplot(1, 1, 1)
#ax.set_yscale("log")
plt.errorbar(xdata, ydata, yerr=yerror, fmt='.', label = 'Simulated Data')
plt.plot(xPlotFreq, fitDataFreq, color = 'r', label = "Fit")
plt.xlabel('Positron Momentum [Mev/c]')
plt.ylabel('Frequency of Polarisation Vector Oscillation [rad/$\mu$s]')
plt.legend(numpoints=1)
plt.savefig(outputDirTop + 'freq_momFit.png')     

# Output fit parameters
print("Fitted parameters: c = {0:.4f}".format(cFreq))
print("Parameter errors: d_c = {0:.2f}".format(d_cFreq))


# Calculate chis**2 per point, summed chi**2 and chi**2/NDF
chiarr = calcChiSq(xFreq, xdata, ydata, xerr = xerror, yerr = yerror)**2
chisq = np.sum(chiarr)
NDF = nPoints - nPars
chisqndf = chisq/NDF
print("ChiSq = {:5.2e}, ChiSq/NDF = {:5.2f}.".format(chisq, chisqndf)) 
...
we = cFreq*10**6 #0.1896E6 #2*np.pi / (40E-6) #-s   
weErr = d_cFreq*10**6
